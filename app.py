# app.py — Crypto Monitor (simple UI, adaptive risk, 0–100 sentiment, ARIMA w/ statsmodels)
...
# ========== Data (cached with dynamic cache‑busters) ==========
...
# ---- Adaptive risk (Low/Med/High) + Risk Score 0–100 ----
def assign_risk(rows: List[dict]) -> List[dict]:
    if not rows: return rows
    df=pd.DataFrame(rows)
    if "atr_pct" not in df or "dollar_volume" not in df: return rows
    ...
    risk_score = 50.0 if rmax==rmin else 100.0*(risk_index - rmin)/(rmax - rmin)
    df["risk_score"]=risk_score

    q1,q2 = np.nanquantile(risk_index,[0.33,0.66]) if len(df)>2 else (np.nan, np.nan)
    df["bucket"]=np.where(risk_index<=q1, "Low",
                    np.where(risk_index<=q2, "Medium","High"))
    ...
    for d in range(max_d+1):
        for p in range(max_p+1):
            for q in range(max_q+1):
                ...
                try:
                    res=ARIMA(y, order=(p,d,q)).fit()  # safe: no disp/method args
...
with tab_overview:
    ...
    for sym in watch:
        try:
            ...
            rows.append({
                ...
                "Dollar Volume": dv,
                "Entry": None, "TP": None, "SL": None,   # fill after risk assignment
            })
        except Exception as e:
            st.warning(f"{sym}: {e}")
    ...
    if rows:
        ...
        for r in rows:
            if pd.notna(r["ATR"]):
                br = bracket_from_atr(r["Last"], r["ATR"], r["bucket"])
        ...
        for i,b in enumerate(["Low","Medium","High"]):
            with cols[i]:
                ...
                if sub.empty:
                    ...
                    # optional position sizing (uses advanced settings)
...
with tab_live:
    st.markdown("## Live")
    # Inject simple JS auto-reload (no extra deps)
    if rate_sec:
...
with tab_predict:
    ...
    with right:
        st.caption("Advanced (from sidebar): sentiment impact & bias floor")
    ...
    if sym:
        ...
        b_disp = to_0_100(b_coin)                 # 0..100
        w = float(st.session_state["sent_w"])
        floor = float(st.session_state["bias_floor"])
        ...
        st.metric("Predicted change", f"{(end/last-1.0)*100:+.2f}%")

        with st.expander("Coin-related headlines used"):
            base=sym.split("/")[0].upper(); keys=COIN_ALIASES.get(base,[base.lower()])
