...
SHEET_HEADERS = [
    ...
    "sentiment_bias","orderbook_imbalance","risk_bucket","entry","tp","sl",
...
@st.cache_data(show_spinner=False)
def order_book_imbalance(ex_id: str, symbol: str, limit: int=20, _ttl_key:int=0) -> float:
    try:
        ex = make_exchange(ex_id)
...
def composite_score(dfi: pd.DataFrame, news_bias: float=0.0, ob_imbalance: float=0.0) -> float:
    ...
    s += 0.10*np.tanh(news_bias)
    s += 0.10*np.tanh(ob_imbalance*2)
